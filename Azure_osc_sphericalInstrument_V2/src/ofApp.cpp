...
void ofApp::setup() {
    ...
    sender_1.setup("127.0.0.1", 9004);  // Spherical Soundfile Player  send to local host
...
void ofApp::update() {
    ...
    if (useLiveTracking) {
        ...
        if (device.get_capture(&liveCapture, std::chrono::milliseconds(0))) {
    ...
    if (tracker.pop_result(&bodyFrame, std::chrono::milliseconds(0))) {
        ...
        if (numBodies > 0) {
            k4abt_body_t body = bodyFrame.get_body(0);  // limited to first tracked body
            ...
            for (int i = 0; i < static_cast<int>(K4ABT_JOINT_COUNT); ++i) {
                ...
                // mirroring on x axis, depending on Projection or Screen (M)
