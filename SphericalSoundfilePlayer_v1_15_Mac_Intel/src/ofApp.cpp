...
//--------------------------------------------------------------
void ofApp::updateOscSender() noexcept(false)
{
    try
    {
...
void ofApp::update()
{
    if (pendingPresetLoad)
    {
        presetControl_GUI->getTextInput("SEL_PRST_NR")->setText(pendingPresetStr);
        mGuiSelectPresetNumber = pendingPresetStr;

        ofxDatGuiButton* btn = presetControl_GUI->getButton("READ_WRITE_PRST");
        if (btn != nullptr) {
            ofxDatGuiButtonEvent fakeEvent(btn);
            onButtonEvent(fakeEvent);
        }

        mAudioTrigger = pendingAudioTrigger;
        std::cout << pendingAudioTrigger << std::endl;
...
void ofApp::loadSoundFolderByIndex(int index, bool forceReload = false, int xRes = 10, int yRes = 10) 
{
	...
	std::string folderPath = ofToDataPath(folderName);
