...
class MotionSynthesis():
    ...
    def setClusterLabel(self, label):
        ...
        cluster_mocap_excerpts = self.model.get_cluster_mocap_excerpts(self.cluster_label)

        if cluster_mocap_excerpts is None:
            return
        
        self.mocap_excerpts = torch.from_numpy(cluster_mocap_excerpts)
        self.mocap_excerpt_count = self.mocap_excerpts.shape[0]
