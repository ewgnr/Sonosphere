...
class MotionSynthesis():
    ...
    def __init__(self, config):
        ...
        self.weight_files = config["weight_files"]
        self.current_weight_file = self.weight_files[0]
    ...
    def setModel(self, model_index):
        
        if model_index >= len(self.weight_files):
            return
        
        model_weights_file = self.weight_files[model_index]
        
        if self.current_weight_file == model_weights_file:
            return
        
        
        self.model.load_state_dict(torch.load(model_weights_file))
        
        print("load state dict ", model_weights_file)
        
        self.current_weight_file = model_weights_file    
